...
import { usePutCommunity } from '../hooks/api-hooks.tsx';

export default function BannerEditor({ community, onCancel, onSaved }) {
  const editorRef = useRef();
  const [image, setImage] = useState(null);
  const [file, setFile] = useState(null);
  const [scale, setScale] = useState(1);
  const [rotate, setRotate] = useState(0);
  const { mutate: updateCommunity, loading, error } = usePutCommunity({
    id: community.uuid,
  });
  ...
  useEffect(() => {
    if (community && community.banner) {
      dataUrlToFile(community.banner)
  ...
  const hasNewBanner = !!file || (!file && (rotate !== 0 || scale !== 1));
        ...
        <Button
          ...
          onClick={() => {
            ...
            updateCommunity(data)
              .then(resp => {
                let updatedCommunity = resp.data;
                alert('Community updated successfully.');
                return onSaved(updatedCommunity);
              })
              .catch(err => alert(`An error occurred: ${err.message}`));
          }}
        >
