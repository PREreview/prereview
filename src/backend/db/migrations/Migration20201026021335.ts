...
export class Migration20201026021335 extends Migration {

  async up(): Promise<void> {
    this.addSql('create table `user` (`id` integer not null primary key autoincrement, `created_at` datetime not null, `updated_at` datetime not null, `username` varchar not null, `name` varchar not null, `email` varchar not null, `orcid` varchar not null);');
    this.addSql('create unique index `user_username_unique` on `user` (`username`);');
    this.addSql('create unique index `user_email_unique` on `user` (`email`);');

    this.addSql('create table `tag` (`id` integer not null primary key autoincrement, `created_at` datetime not null, `updated_at` datetime not null, `name` varchar not null, `color` varchar null);');
    ...
    this.addSql('create table `preprint_tags` (`preprint_id` integer not null, `tag_id` integer not null, primary key (`preprint_id`, `tag_id`));');
    this.addSql('create index `preprint_tags_preprint_id_index` on `preprint_tags` (`preprint_id`);');
    this.addSql('create index `preprint_tags_tag_id_index` on `preprint_tags` (`tag_id`);');
    ...
    this.addSql('create table `rapid_review` (`id` integer not null primary key autoincrement, `created_at` datetime not null, `updated_at` datetime not null, `contents` varchar not null);');
    ...
    this.addSql('create table `persona_full_reviews` (`persona_id` integer not null, `full_review_id` integer not null, primary key (`persona_id`, `full_review_id`));');
    this.addSql('create index `persona_full_reviews_persona_id_index` on `persona_full_reviews` (`persona_id`);');
    this.addSql('create index `persona_full_reviews_full_review_id_index` on `persona_full_reviews` (`full_review_id`);');
    ...
    this.addSql('create table `comment` (`id` integer not null primary key autoincrement, `created_at` datetime not null, `updated_at` datetime not null, `title` varchar not null, `contents` varchar not null);');
